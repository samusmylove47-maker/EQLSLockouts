...
const RAIDS = Object.freeze([
  ...
  },
  {
    ...
    //
    // Hate is the one zone where the question is live and reader-facing. Both
    // of this row's bosses have been seen ONLY in the group shape; the raid
    // shape was entered 10 times and produced neither. Whether the two shapes
    // share a lock is UNMEASURED and cannot be settled from our corpus — the
    // alt+Z window was taken after Group runs, so it could only ever have
    // returned the group shape. See note 2 above RAIDS.
    //
    // Carries no corpus count: 10 visits is a fact about OUR logs, and this
    // string is read by someone looking at THEIRS.
    shapeNote: 'both of these have only ever been seen dying in the GROUP ' +
               'instance — whether the raid shape (“The Plane of Hate N”) ' +
               'shares this lock is NOT KNOWN, so read this cell as the group one',
  },
].map(Object.freeze));
