...
//
// Raised by the Director from the modelling session, and MEASURED HERE rather
// than taken on trust. Over the live log, keying on
// `<target> has taken <N> damage from <spell> by <caster>` and taking the eight
// sources with >=40 samples and a dominant value:
//
...
//
// Every single below-modal observation in the sample is a killing blow, against
// a 1.7% base rate. A ~59x difference. The effect is real.
//
// SO: if you ever model damage, filter out any value whose target dies at the
// same timestamp before treating it as a measurement. The kill lines below are
// exactly what you need to do that, which is the one good reason to read them
// together.
