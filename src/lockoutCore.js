...
//
// 2. **THE PLANE OF HATE ROW DESCRIBES THE GROUP INSTANCE.** The alt+Z window
...
//
//    ~~"Ten further bosses die in Hate instances but only in the bare `Zone N`
//    shape. The two shapes hold different populations."~~ **REFUTED 3 Sep 2026,
//    by me, against my own claim.** raids-measured.json holds 12 distinct Hate
//    bosses over 65 rows. Split on its own `group_instance` flag:
//
//        only in the group shape  2   Innoruuk, Maestro of Rancor
//        only in the raid shape   1   High Priest M`kari
//        in BOTH shapes           9
//
//    **Nine of the ten "further" bosses die in the group shape as well.** The
//    two shapes hold LARGELY THE SAME population, not different ones. An
//    independent pass over the 16 raw logs, classifying through this module's
//    own `parseLine`, agrees in structure: 12 of 15 distinct named mobs appear
//    in both, the same 2 group-only, 1 raid-only.
//
//    The count of ten was right and the exclusivity was invented. It read as
//    the strongest argument for splitting this row in two, and it was the one
//    part of it never measured.
//
//    **WHAT SURVIVES, AND IT IS SHARPER.** The two bosses this row is keyed on
//    are exactly the two never once seen in the raid shape — across 10 raid-
//    shape Hate visits, neither died. So the raid shape has never produced a
//    completion for this row. **That is still not a lockout fact:** a shared
//    population does not imply a shared lock, and neither does a disjoint one.
//    Whether the shapes lock together is unmeasured and stays unmeasured.
//
...
const RAIDS = Object.freeze([
  ...
  },
  {
    ...
    singleBoss: false,
    weeklyTaskObserved: false,
    // WHICH INSTANCE THIS ROW IS ABOUT, on the page rather than in a comment.
    //
    // ANSWERED 3 Sep 2026 by the owner, asked directly:
    //   "No, they are not shared. Each hate raid instance is unique and separate."
    //
    // So the group and raid shapes do NOT share a lock, and this row describes
    // the group one. That is settled — but it is `stated`, NOT `observed`, and
    // the page must not dress it as a measurement. We could not have measured
    // it: zero roster kills have ever occurred in a raid-shape instance, so we
    // have no completion data for raid content to reason from. Same provenance
    // class as RESET_RULE, which says `stated` for the same kind of reason.
    //
    // Carries no corpus count: "10 raid-shape visits" is a fact about OUR logs,
    // and this string is read by someone looking at THEIRS.
    shapeNote: 'this cell is the GROUP instance. The raid shape ' +
               '(“The Plane of Hate N”) is a SEPARATE lockout and is not ' +
               'tracked here — owner, stated, not measured by us',
  },
].map(Object.freeze));
...
function createState(character, opts) {
  ...
  return {
    ...
    raids: raids.map((r) => ({
      ...
      // Which instance shape the row describes, where that is in doubt. Null
      // for rows with nothing to say. Copied so a restored snapshot renders
      // the same tooltip as a live one.
      shapeNote: r.shapeNote || null,
    })),
  ...
  for (const entry of state.raids) {
    ...
    for (let d = 0; d < DIFFICULTY_LABELS.length; d++) {
      ...
      cells.push({
        ...
        // Which instance shape this row describes, where that is in doubt.
        // Null for rows with nothing to say — see the Hate entry in RAIDS.
        shapeNote: entry.shapeNote || null,
        difficulty: d,
